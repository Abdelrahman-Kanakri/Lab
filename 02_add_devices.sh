#!/usr/bin/env bash
# add_devices.sh - find newly-enrolled lab devices and add them to MeshCentral
#
# Run this AFTER walking around with the Enroll-LabDevice.bat USB.
# It will:
#   1. Scan the lab subnet for hosts with WinRM open (in parallel batches)
#   2. Diff against ~/lab/hosts.ini to find what's new
#   3. Test auth on the new ones
#   4. Show you the list and ask to proceed
#   5. Run the enrollment playbook on the confirmed-working ones
#   6. Verify enrollment via PLAY RECAP and merge successes into ~/lab/hosts.ini

set -euo pipefail
source "$(dirname "$0")/config.env"

# Derive the /24 prefix from LAB_RANGE_START in config.env (e.g. "10.3.5.1" -> "10.3.5")
SUBNET="${LAB_RANGE_START%.*}"

TMP_FOUND="/tmp/lab_scan_found.txt"
TMP_NEW="/tmp/lab_scan_new.txt"
TMP_READY="/tmp/lab_scan_ready.txt"
TMP_INI="/tmp/lab_scan_inventory.ini"

# Build an inventory file for ad-hoc steps below using config.env values
write_inventory() {
    local hosts_file="$1" out="$2"
    {
        echo "[lab]"
        cat "$hosts_file"
        echo ""
        echo "[lab:vars]"
        echo "ansible_user=$LAB_ADMIN_USER"
        echo "ansible_password=$LAB_ADMIN_PASS"
        echo "ansible_connection=winrm"
        echo "ansible_winrm_transport=ntlm"
        echo "ansible_winrm_server_cert_validation=ignore"
        echo "ansible_port=5985"
    } > "$out"
}

echo "================================================================"
echo "  Lab Device Discovery & Enrollment"
echo "================================================================"
echo ""

# ---- Step 1: scan (parallel, batches of 50) ----
echo "[1/6] Scanning $SUBNET.0/24 for WinRM (5985)..."
> "$TMP_FOUND"

for i in $(seq 1 254); do
    ip="$SUBNET.$i"
    [ "$ip" = "$CONTROLLER_IP" ] && continue

    # Run the connection check in the background
    (
        if timeout 3 bash -c "echo > /dev/tcp/$ip/5985" 2>/dev/null; then
            echo "$ip" >> "$TMP_FOUND"
        fi
    ) &

    # Batch limit: wait for every 50 background jobs before continuing
    if (( i % 50 == 0 )); then
        wait
    fi
done
# Wait for the last partial batch
wait

TOTAL_FOUND=$(wc -l < "$TMP_FOUND")
echo "    found $TOTAL_FOUND hosts with WinRM open"

if [ "$TOTAL_FOUND" -eq 0 ]; then
    echo ""
    echo "No WinRM hosts found. Check that you ran Enroll-LabDevice.bat on devices."
    exit 1
fi

# ---- Step 2: diff against existing inventory ----
echo ""
echo "[2/6] Finding new devices (not yet in ~/lab/hosts.ini)..."
ALREADY="/tmp/lab_already.txt"
grep -oE "^$SUBNET\.[0-9]+" ~/lab/hosts.ini 2>/dev/null | sort -u > "$ALREADY"
comm -23 <(sort "$TMP_FOUND") "$ALREADY" > "$TMP_NEW"
NEW_COUNT=$(wc -l < "$TMP_NEW")
echo "    $NEW_COUNT new devices:"
sort -t. -k4 -n "$TMP_NEW" | sed 's/^/      /'

if [ "$NEW_COUNT" -eq 0 ]; then
    echo ""
    echo "Nothing new to enroll. All scanned devices are already in inventory."
    exit 0
fi

# ---- Step 3: test auth (live output so you can see progress) ----
echo ""
echo "[3/6] Testing $LAB_ADMIN_USER auth on the $NEW_COUNT new hosts..."
echo "      (Running Ansible in real-time so you can see exactly what is happening...)"

write_inventory "$TMP_NEW" "$TMP_INI"

ansible lab -i "$TMP_INI" -m win_ping --forks 20 | tee /tmp/lab_auth_log_raw.txt

# Extract SUCCESS hosts from the live log
grep "SUCCESS" /tmp/lab_auth_log_raw.txt | awk '{print $1}' | sort -u > "$TMP_READY"
READY_COUNT=$(wc -l < "$TMP_READY")
SKIPPED_COUNT=$(grep -E -c "UNREACHABLE|FAILED" /tmp/lab_auth_log_raw.txt || true)

echo ""
echo "    auth OK on $READY_COUNT  /  failed on $SKIPPED_COUNT"
if [ "$SKIPPED_COUNT" -gt 0 ]; then
    grep -E "UNREACHABLE|FAILED" /tmp/lab_auth_log_raw.txt | awk '{print "      [skip] " $1}'
    echo ""
    echo "    (skipped hosts likely need Enroll-LabDevice.bat re-run, or are non-lab)"
fi

if [ "$READY_COUNT" -eq 0 ]; then
    echo ""
    echo "No new devices passed auth. Aborting."
    exit 1
fi

# ---- Step 4: confirm ----
echo ""
echo "[4/6] Ready to enroll these in MeshCentral:"
sort -t. -k4 -n "$TMP_READY" | sed 's/^/      /'
echo ""
read -r -p "Proceed with enrollment? [y/N] " ans
if [[ ! "$ans" =~ ^[Yy]$ ]]; then
    echo "Cancelled."
    exit 0
fi

# ---- Step 5: enroll (live output) ----
echo ""
echo "[5/6] Running enrollment playbook..."

write_inventory "$TMP_READY" "$TMP_INI"

ansible-playbook -i "$TMP_INI" ~/lab/playbooks/01_enroll_with_unlock.yml \
    --forks "$READY_COUNT" | tee /tmp/lab_enroll_log.txt

# ---- Step 6: verify via PLAY RECAP and merge into hosts.ini ----
echo ""
echo "[6/6] Verifying enrollment and merging into ~/lab/hosts.ini..."

# Split PLAY RECAP into success (unreachable=0 AND failed=0) and failed (everything else)
: > /tmp/lab_enroll_success.txt
: > /tmp/lab_enroll_failed.txt
awk -v subnet="$SUBNET" '
    /PLAY RECAP/ { flag = 1; next }
    flag && $1 ~ "^"subnet"\\.[0-9]+" {
        if ($0 ~ /unreachable=0/ && $0 ~ /failed=0/) {
            print $1 > "/tmp/lab_enroll_success.txt"
        } else {
            print $0 > "/tmp/lab_enroll_failed.txt"
        }
    }
' /tmp/lab_enroll_log.txt

ENROLLED_COUNT=$(wc -l < /tmp/lab_enroll_success.txt)
FAILED_COUNT=$(wc -l < /tmp/lab_enroll_failed.txt)

if [ "$FAILED_COUNT" -gt 0 ]; then
    echo "  Failed enrollments ($FAILED_COUNT) — NOT added to hosts.ini:"
    sed 's/^/      /' /tmp/lab_enroll_failed.txt
    echo "  (full log: /tmp/lab_enroll_log.txt   failed list: /tmp/lab_enroll_failed.txt)"
    echo ""
fi

if [ "$ENROLLED_COUNT" -eq 0 ]; then
    echo "  No devices successfully enrolled. Check the playbook errors above."
    exit 1
fi

echo "  Successfully enrolled $ENROLLED_COUNT device(s)."

# Merge verified successes into hosts.ini
ALL_HOSTS="/tmp/lab_all_hosts.txt"
{
    grep -oE "^$SUBNET\.[0-9]+" ~/lab/hosts.ini 2>/dev/null
    cat /tmp/lab_enroll_success.txt
} | sort -u -t. -k4 -n > "$ALL_HOSTS"

{
    echo "# Auto-generated $(date -Iseconds) - lab devices verified enrolled in MeshCentral"
    echo ""
    echo "[lab]"
    cat "$ALL_HOSTS"
    echo ""
    echo "[lab:vars]"
    echo "ansible_user=$LAB_ADMIN_USER"
    echo "ansible_password=$LAB_ADMIN_PASS"
    echo "ansible_connection=winrm"
    echo "ansible_winrm_transport=ntlm"
    echo "ansible_winrm_server_cert_validation=ignore"
    echo "ansible_port=5985"
} > ~/lab/hosts.ini

TOTAL=$(wc -l < "$ALL_HOSTS")
echo ""
echo "================================================================"
echo "  Done. ~/lab/hosts.ini now has $TOTAL devices."
echo "  Open https://$CONTROLLER_IP to see them in MeshCentral."
echo "================================================================"
